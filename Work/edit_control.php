...
<?php
    if (isset($_REQUEST['id'])) {
        $id = $_REQUEST['id'];
        $select_stmt = $db->prepare("SELECT * FROM drugs WHERE drug_id = :id");
...
<body >
 ...
 <nav class="navbar navbar-expand-lg bg-light">
  ...
  <h1 class="text mt-5" style="text-align:center;">♡แก้ไขข้อมูล♡</h1>
      ...
      <div class="col-md-12">
        <div class="card" >
          ...
          <div style="display: flex; align-items: center;">
            <form style="flex-basis: 50%;" method="post" action="edit_control_db.php">
              <h1 class="card-title ">Control</h1>
              <label for="channel" class="form-label">Channel(เลือกช่องยา)</label>
              <div class="mb-3 ">
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="channel" id="inlineRadio1" <?php if ($row['channel'] == 1) { echo "checked";} ?> value="1">
                    <label class="form-check-label" for="inlineRadio1">1</label>
                  </div>
                  <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="channel" id="inlineRadio2" <?php if ($row['channel'] == 2) { echo "checked";} ?> value="2">
                    <label class="form-check-label" for="inlineRadio2">2</label>
                  </div>
                  <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="channel" id="inlineRadio2" <?php if ($row['channel'] == 3) { echo "checked";} ?> value="3">
                    <label class="form-check-label" for="inlineRadio3">3</label>
                  </div>
                  <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="channel" id="inlineRadio2" <?php if ($row['channel'] == 4) { echo "checked";} ?> value="4">
                    <label class="form-check-label" for="inlineRadio4">4</label>
                  </div>
                  <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="channel" id="inlineRadio2" <?php if ($row['channel'] == 5) { echo "checked";} ?> value="5">
              ...
              <div class="mb-3 ">
                ...
                <input  type="text" id="namedrug" placeholder="ใส่ชื่อยา" name="drug_name" value="<?php echo $row['drug_name']; ?>" style="width: 100%;"  class="form-control" required  aria-describedby="emailHelp">
              </div>

              <div class="mb-3 ">
                <label for="settime" class="form-label">Set Time</label>
                <input  type="time" id="settime"style="width: 100%;" name="time" value="<?php echo $row['time']; ?>"  class="form-control" required  aria-describedby="emailHelp">
              </div>
              <a>การรับประทานยา</a>
              <div class="form-check">
                <input class="form-check-input" type="radio" name="time_to_eat" value="before" <?php if ($row['time_to_eat'] == "before") { echo "checked";} ?>  id="flexRadioDefault1">
                <label class="form-check-label" for="flexRadioDefault1">ก่อนอาหาร</label>
              </div>

              <div class="form-check ">
                <input class="form-check-input" type="radio" name="time_to_eat" value="after" <?php if ($row['time_to_eat'] == "after") { echo "checked";} ?> id="flexRadioDefault2">
                <label class="form-check-label" for="flexRadioDefault2">หลังอาหาร</label>
              </div>

              <div class="mb-3 ">
                <label for="settime" class="form-label">Exp Date</label>
                <input  type="date" id="settime"style="width: 100%;" name="exp" value="<?php echo $row['exp']; ?>" required  class="form-control"  aria-describedby="emailHelp">
              </div>


            
              <!--  เมื่อคลิ๊ก ยกเลิก ให้ไปหน้าData Drug เพื่อโชว์ข้อมูลที่กรอกก่อนหน้า -->
              <!-- <button  class="button_login1" onclick="location.href='/Work/index.html'"  ><a>ยกเลิก</a> -->
